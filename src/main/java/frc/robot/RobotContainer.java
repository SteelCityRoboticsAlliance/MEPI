...
public class RobotContainer {
  ...
  private final TowerKickerCommand m_towerKickerCommand = new TowerKickerCommand(m_towerSubsystem);
  private final ShooterPIDCommand m_shooterPIDCommand =
      new ShooterPIDCommand(m_shooterSubsystem, ShooterSubsytem.FENDER_RPM);
  ...
  private void configureButtonBindings() {
    ...
    //  new Button(() -> m_controlJoystick.getRightY() > 0.75).whileHeld(new
    // ClimberCommand(m_climberSubsystem, 0.25));
    //  new Button(() -> m_controlJoystick.getRightY() < -0.75).whileHeld(new
    // ClimberCommand(m_climberSubsystem, -0.25));
    new JoystickButton(m_controlJoystick, XboxController.Button.kLeftBumper.value)
        .whenHeld(m_shooterPIDCommand).whenReleased(new SetShooterSpeedCommand(m_shooterSubsystem, 0));
    new JoystickButton(m_controlJoystick, XboxController.Button.kRightBumper.value)
        .whenHeld(m_kickIfShootSetRPMCommand);
